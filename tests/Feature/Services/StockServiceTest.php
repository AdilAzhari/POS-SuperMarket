...
it('returns zero stock when product not attached to store', function (): void {
    $stock = $this->stockService->getStockForStore($this->product->id, $this->store->id);

    expect($stock)->toBe(0);
});

it('returns correct stock when product attached to store', function (): void {
...
it('prevents stock from going below zero when decrementing', function (): void {
    $this->product->stores()->attach($this->store->id, [
        'stock' => 2,
        'low_stock_threshold' => 5,
    ]);
...
it('creates pivot record when decrementing stock for unattached product', function (): void {
...
it('can increment stock', function (): void {
    $this->product->stores()->attach($this->store->id, [
        'stock' => 10,
        'low_stock_threshold' => 5,
    ]);
...
it('creates pivot record when incrementing stock for unattached product', function (): void {
    // Product not attached to store initially
    expect($this->product->stores()->count())->toBe(0);
