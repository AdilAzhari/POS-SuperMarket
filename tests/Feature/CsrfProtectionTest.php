...
test('csrf middleware is properly loaded', function () {
...
test('csrf protection works with database sessions', function () {
...
test('csrf protection allows valid tokens', function () {
...
test('login endpoint has csrf protection', function () {
...
test('csrf token generation works in testing environment', function () {
    config(['session.driver' => 'database']);
    ...
    expect(strlen($token))->toBeGreaterThan(30); // CSRF tokens are typically long
});

// Helper to inspect middleware configuration
test('inspect middleware configuration', function () {
    $app = app();
    $router = $app['router'];
