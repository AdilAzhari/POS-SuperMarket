...
test('csrf protection works in real web environment', function () {
...
test('csrf token is properly generated in meta tag', function () {
    config(['session.driver' => 'database']);
    ...
    if (preg_match('/name="csrf-token"\s+content="([^"]+)"/', $content, $matches)) {
        ...
        expect(strlen($metaToken))->toBeGreaterThan(30);
    } else {
        throw new Exception('CSRF token not found in meta tag');
    }
});

test('inertia csrf token sharing works', function () {
    config(['session.driver' => 'database']);
...
test('check if there are csrf exclusions', function () {
