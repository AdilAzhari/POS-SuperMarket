<template>
    <div class="flex flex-col lg:flex-row gap-4 lg:gap-6 h-screen lg:h-auto p-4 lg:p-0">
        ...
        <div class="flex-1 lg:flex-none lg:w-2/3 space-y-4 lg:space-y-6 min-h-0">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 lg:p-6">
                <div class="space-y-3">
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="w-full pl-12 pr-16 py-3 text-lg border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            autocomplete="off"
                            @keydown.enter="quickAddProduct"
                        />
                        <button
                            @click="showBarcodeScanner = !showBarcodeScanner"
                            class="absolute right-2 top-2 p-2 text-gray-400 dark:text-gray-500 hover:text-blue-600 transition-colors"
                    ...
                    <div v-if="showBarcodeScanner" class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                        <BarcodeScanner
                ...
                <!-- Inventory Alerts -->
                <div v-if="inventoryAlerts.length > 0" class="mt-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg p-3">
                ...
                {{ inventoryAlerts.length }} Low Stock Alert{{ inventoryAlerts.length > 1 ? 's' : '' }}
                    ...
                    <div v-if="showInventoryAlerts" class="mt-2 space-y-1">
                        ...
                        >
                            <span class="text-yellow-700 dark:text-yellow-300">{{ alert.name }}</span>
                            <span :class="alert.is_critical ? 'text-red-600 font-bold' : 'text-yellow-600'">
                {{ alert.current_stock }} left
              </span>
                        </div>
                        <div v-if="inventoryAlerts.length > 5" class="text-xs text-yellow-600 dark:text-yellow-400 text-center pt-1">
                ...
                <div class="mt-4 lg:mt-6 flex space-x-3 overflow-x-auto scrollbar-hide pb-2">
                    <button
                        ...
                        :class="selectedCategory === '' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'"
                    ...
                    <button
                        ...
                        :class="selectedCategory === category ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'"
                        class="px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors"
                    >
                        {{ category }}
                    </button>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 lg:p-6 flex-1 min-h-0 overflow-hidden">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 lg:mb-6">Products</h3>
                <div class="h-full overflow-y-auto -mx-2">
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-3 lg:gap-4 px-2">
                        <button
                            ...
                            :key="p.id"
                            class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-left hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:ring-2 hover:ring-blue-500 transition-all active:scale-95 touch-manipulation"
                            @click="pos.addToCart(p)"
                        >
                            <img :src="p.image" :alt="p.name" class="w-full h-24 md:h-28 object-cover rounded-md mb-2" />
                            <div class="font-medium text-gray-900 dark:text-white text-sm md:text-base truncate">
                                {{ p.name }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ p.sku }} • {{ p.category }}</div>
                            <div class="flex justify-between items-center mt-1">
                                <div class="font-semibold text-blue-600 text-sm md:text-base">${{ p.price.toFixed(2) }}</div>
                                <div class="text-xs text-green-600 font-medium">{{ p.stock }} in stock</div>
                            </div>
                        </button>
                    </div>
                </div>
        ...
        <div class="lg:w-1/3 space-y-4 lg:space-y-6 flex flex-col min-h-0">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 lg:p-6 flex-1 min-h-0 overflow-hidden">
                <div class="flex items-center justify-between mb-4 lg:mb-6">
                    <h3 class="font-semibold text-gray-900 dark:text-white text-lg lg:text-xl">Cart</h3>
                    <button
                ...
                <div v-if="pos.cartItems.length === 0" class="flex items-center justify-center h-32 text-gray-500 dark:text-gray-400">
                ...
                <div v-else class="h-full overflow-y-auto -mx-2 px-2 space-y-3 lg:space-y-4">
                    <div
                        ...
                        :key="item.id"
                        class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 lg:p-4 space-y-3"
                    >
                        <div class="flex items-start justify-between">
                            ...
                            <div class="text-right">
                                <p class="font-medium text-gray-900 dark:text-white">${{ item.lineTotal.toFixed(2) }}</p>
                            </div>
                        </div>
                        ...
                        <!-- Quantity Controls -->
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-3 bg-white dark:bg-gray-600 rounded-lg p-1">
                                <button
                        ...
                        <div class="flex items-center justify-between mt-2">
                            <span class="text-xs text-gray-500">Item Discount:</span>
                            <div class="flex items-center space-x-2">
                                <div class="flex items-center space-x-1">
                                    <span class="text-xs">$</span>
                                    <input
                                        :value="item.discount || 0"
                                        @input="updateItemDiscount(item.id, $event.target.value)"
                                        type="number"
                                        min="0"
                                        :max="item.price * item.quantity"
                                        step="0.01"
                                        class="w-16 px-1 py-0.5 text-xs border rounded focus:ring-1 focus:ring-blue-500"
                                        placeholder="0.00"
                                    />
                                </div>
                                <button
                                    @click="showPromotionsModal(item)"
                ...
                <div v-if="pos.selectedCustomer && availableLoyaltyRewards.length > 0" class="mt-4 lg:mt-6 border-t pt-4">
                    <h4 class="text-sm font-semibold text-gray-900 mb-3 flex items-center">
                        <svg class="w-4 h-4 mr-2 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
                        Available Rewards ({{ pos.selectedCustomer.loyalty_points }} pts)
                    </h4>
                    <div class="space-y-2 max-h-32 overflow-y-auto">
                        <button
                            v-for="reward in availableLoyaltyRewards"
                            :key="reward.id"
                            @click="applyLoyaltyReward(reward)"
                            :disabled="appliedLoyaltyReward?.id === reward.id"
                            class="w-full text-left p-2 border rounded-lg hover:bg-blue-50 disabled:bg-gray-100 disabled:cursor-not-allowed transition-colors"
                        >
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    <div class="text-xs font-medium text-gray-900">{{ reward.name }}</div>
                                    <div class="text-xs text-gray-500">{{ reward.points_required }} pts • {{ formatRewardValue(reward) }}</div>
                                </div>
                                <div v-if="appliedLoyaltyReward?.id === reward.id" class="text-green-500 text-xs">✓ Applied</div>
                            </div>
                        </button>
                    </div>
                    <button
                        v-if="appliedLoyaltyReward"
                        @click="removeLoyaltyReward()"
                ...
                <div class="mt-4 lg:mt-6 border-t pt-4 lg:pt-6 space-y-2 lg:space-y-3 text-sm lg:text-base">
                    ...
                    <div v-if="loyaltyDiscount > 0" class="flex justify-between text-purple-600">
    ...
    <!-- Promotions Modal -->
    <div v-if="showPromotions" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div class="p-4 border-b flex items-center justify-between">
                <h3 class="text-lg font-semibold">Apply Promotion</h3>
                <button @click="closePromotionsModal" class="text-gray-500 hover:text-gray-700">
                    <X class="w-6 h-6" />
                </button>
            </div>
            <div class="p-4">
                <div v-if="selectedCartItem" class="mb-4">
                    <h4 class="font-medium">{{ selectedCartItem.name }}</h4>
                    <p class="text-sm text-gray-500">${{ selectedCartItem.price }} × {{ selectedCartItem.quantity }}</p>
                </div>

                <!-- Quick Discount Presets -->
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Quick Discounts</label>
                    <div class="grid grid-cols-3 gap-2">
                        <button
                            v-for="preset in discountPresets"
                            :key="preset.value"
                            @click="applyDiscountPreset(preset.value, preset.type)"
                ...
                <!-- Custom Discount -->
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Custom Discount</label>
                    <div class="flex items-center space-x-2">
                        <input
                            v-model="customDiscountAmount"
                            type="number"
                            min="0"
                            :max="selectedCartItem ? selectedCartItem.price * selectedCartItem.quantity : 0"
                            step="0.01"
                ...
                <!-- Current Discount -->
                <div v-if="selectedCartItem && selectedCartItem.discount > 0" class="mb-4 p-3 bg-green-50 rounded-lg">
                    <div class="flex items-center justify-between">
            <span class="text-sm text-green-700">
              Current discount: ${{ selectedCartItem.discount.toFixed(2) }}
            </span>
                        <button
...
// Loyalty and promotions state
const availableLoyaltyRewards = ref([])
...
const customDiscountType = ref('fixed')

// Discount presets
const discountPresets = ref([
...
// Payment state - must be declared before onMounted
    ...
    try {
        ...
        try {
            ...
            paymentMethods.value = response.data.methods || [
                { code: 'cash', name: 'Cash', icon: '💵', fee_rate: 0 },
                { code: 'card', name: 'Credit/Debit Card', icon: '💳', fee_rate: 2.5 },
                { code: 'digital', name: 'Digital Wallet', icon: '📱', fee_rate: 1.5 }
            ]
        } catch (error) {
            console.warn('Failed to load payment methods, using defaults:', error)
...
const filteredProducts = computed(() => {
    ...
    // Filter out products with insufficient stock (stock must be greater than 0)
...
// Remove duplicate onMounted - handled above
...
// Final cart total with loyalty discounts
const finalCartTotal = computed(() => {
    return Math.max(0, pos.grandTotal - loyaltyDiscount.value)
})
...
const quickAddProduct = () => {
    ...
    const product = productsStore.products.find(p => 
        p.barcode === productQuery.value.trim() || 
        p.name.toLowerCase().includes(productQuery.value.toLowerCase())
    )
...
const updateItemDiscount = (itemId, discountValue) => {
    ...
    const item = pos.cartItems.find(i => i.id === itemId)
    if (item) {
        const maxDiscount = item.price * item.quantity
        const finalDiscount = Math.min(discount, maxDiscount)
        pos.updateCartItem(itemId, item.quantity, finalDiscount)
    }
}

const showPromotionsModal = (item) => {
    selectedCartItem.value = item
    customDiscountAmount.value = item.discount || 0
...
const applyDiscountPreset = (value, type) => {
    ...
    if (type === 'percentage') {
        discount = (item.price * item.quantity) * (value / 100)
    } else {
        discount = value
    }

    discount = Math.min(discount, maxDiscount)
    pos.updateCartItem(item.id, item.quantity, discount)
    closePromotionsModal()
}

const applyCustomDiscount = () => {
    if (!selectedCartItem.value || !customDiscountAmount.value) return

    const item = selectedCartItem.value
    const maxDiscount = item.price * item.quantity
    let discount = 0

    if (customDiscountType.value === 'percentage') {
        discount = (item.price * item.quantity) * (customDiscountAmount.value / 100)
    } else {
        discount = customDiscountAmount.value
    }
...
const removeItemDiscount = () => {
    if (!selectedCartItem.value) return
    pos.updateCartItem(selectedCartItem.value.id, selectedCartItem.value.quantity, 0)
    closePromotionsModal()
}
...
const fetchLoyaltyRewards = async () => {
    if (!pos.selectedCustomer) {
        availableLoyaltyRewards.value = []
        return
    }

    try {
        const response = await axios.get(`/api/loyalty/rewards/available/${pos.selectedCustomer.id}`)
        if (response.data.success) {
            availableLoyaltyRewards.value = response.data.rewards || []
        }
    } catch (error) {
...
const applyLoyaltyReward = (reward) => {
    if (appliedLoyaltyReward.value) {
        removeLoyaltyReward()
    }

    appliedLoyaltyReward.value = reward
    const cartSubtotal = pos.grandTotal - pos.tax
    loyaltyDiscount.value = calculateLoyaltyDiscount(reward, cartSubtotal)
}

const calculateLoyaltyDiscount = (reward, subtotal) => {
    switch (reward.type) {
        ...
        case 'free_product':
            return reward.free_product?.price || 0
        default:
            return 0
    }
}
...
// Watch for customer changes to fetch loyalty rewards
watch(() => pos.selectedCustomer, (newCustomer) => {
    if (newCustomer) {
        fetchLoyaltyRewards()
    } else {
        availableLoyaltyRewards.value = []
...
const checkout = async () => {
    ...
    try {
        // First create the sale with loyalty information
        const saleResponse = await pos.checkout({
            storeId: 1,
            paymentMethod: selectedPaymentMethod.value,
            loyaltyReward: appliedLoyaltyReward.value ? {
                id: appliedLoyaltyReward.value.id,
        ...
        if (paymentResponse.data.payment.status === 'completed') {
            ...
            try {
                const saleWithPaymentResponse = await axios.get(`/api/sales/${saleResponse.data.id}`)
